...
export const oddValues = (arr) => {
  ...
  export const filterStringsWithVowels = (arr) => {
    let withVowels = arr.filter(function(w, i) {
      return w.match(/[aeiou]/gi, '')
    })
    return withVowels;
  };
  ...
  export const notInFirstArray = (forbiddenValues, arr) => {
    let notInFirst = arr.filter(n => !forbiddenValues.includes(n));
    return notInFirst;
  };
  ...
  export const getBaseStatGreaterThan = (arr, minBaseStat) => {
    let greater = arr.filter(n => minBaseStat < n.baseStat)
    return greater
  };
  ...
  export const getStatName = (arr, minBaseStat) => {
    let greater = arr.filter(n => minBaseStat < n.baseStat);
    return greater.map(item => item.stat.name);
  };
  ...
  export const getCharactersWithoutChildren = (arr) => {
    let children = arr.filter(n => !n.children);
    return children;
  };
  ...
  export const evenOddNumericValues = (arr) => {
    let newArr = arr.filter(n => Number.isInteger(n));
