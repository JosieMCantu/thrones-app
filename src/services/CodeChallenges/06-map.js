...
export const forLoopTwoToThe = (arr) => {
    ...
    export const mapTwoToThe = (arr) => {
      const squared = arr.map(num => 2 ** num);
      return squared;
    };
    ...
    export const charCode = (arr) => {
      const coded = arr.map(letter => letter.charCodeAt());
      return coded
    };
    ...
    export const extractAbilities = (arr) => {
      // arr.map(item => {
      //   const newArr = item.ability.name;
      //   return newArr;
      // })
    ...
    export const extractStats = (arr) => {
      ...
      return arr.map(item => {
        const total = item.effort + item.baseStat;
        return {name: item.stat.name, total: total};
      });
